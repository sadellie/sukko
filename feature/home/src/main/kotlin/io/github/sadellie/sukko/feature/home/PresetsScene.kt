...
private fun PresetSheetContent(
  ...
  Column(
    ...
    ListItem2(
      headlineContent = { Text(stringResource(Res.string.home_presets_apply_preset_title)) },
      leadingContent = { Icon(Symbols.Check, contentDescription = null) },
      modifier = Modifier.clickable { dialogState = DialogState.APPLY },
      shape = if (customPreset) ListItemDefaults.firstShape else ListItemDefaults.singleShape,
    )
    if (customPreset) {
      ListItem2(
        headlineContent = { Text(stringResource(Res.string.common_rename)) },
        leadingContent = { Icon(Symbols.Edit, contentDescription = null) },
        modifier = Modifier.clickable { dialogState = DialogState.RENAME },
        shape = ListItemDefaults.middleShape,
      )
      ListItem2(
        headlineContent = { Text(stringResource(Res.string.home_presets_export)) },
        leadingContent = { Icon(Symbols.FileExport, contentDescription = null) },
        modifier = Modifier.clickable { dialogState = DialogState.EXPORT },
        shape = ListItemDefaults.middleShape,
      )
      ListItem2(
        ...
        modifier = Modifier.clickable { dialogState = DialogState.DELETE },
  ...
  fun exportPreset(preset: WidgetDataPreset, destination: PlatformFile) {
    viewModelScope.launch {
      ...
      try {
        ...
        Logger.e(TAG, e) { "Failed to export preset with id ${preset.presetId}" }
      }
    }
  }
