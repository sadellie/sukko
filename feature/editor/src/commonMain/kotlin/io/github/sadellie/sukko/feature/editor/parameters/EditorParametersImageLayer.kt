...
private fun EditorParametersContentScale(
  ...
  ListItem2Compact(
    content = { Text(stringResource(Res.string.editor_parameters_content_scale)) },
    supportingContent = { Text(stringResource(layer.contentScale.displayName)) },
    compactListMode = compactListMode,
    onClick = contentScaleSheetState::expand,
    shapes = shapes,
  )
  ...
  if (scaleSource is ContentScaleSource.FixedScale) {
    ...
    ListItem2Compact(
      content = { Text(stringResource(Res.string.editor_parameters_scale_factor)) },
      supportingContent = { Text(LocalScriptableDisplay.current.displayString(scaleSource.scale)) },
      compactListMode = compactListMode,
      onClick = contentScaleFactorState::expand,
      shapes = shapes,
    )
    DoubleSelectorSheet(
...
private fun EditorParametersTint(
  ...
  ListItem2Compact(
    content = { Text(stringResource(Res.string.editor_parameters_tint)) },
    supportingContent = {
