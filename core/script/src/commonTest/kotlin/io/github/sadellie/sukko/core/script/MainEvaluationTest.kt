...
class MainEvaluationTest {
  ...
  @Test fun evaluate_batteryLevel1() = assertEvaluation("batteryLevel / 100", 0.72)

  @Test fun evaluate_batteryLevel2() = assertEvaluation("batteryLevel\"%\"", "72%")
  ...
  @Test fun evaluate_currentDate1() = assertEvaluation("currentDate(\"HH:mm\")", "23:59")

  @Test
  fun evaluate_currentDateWithTimeZone1() =
    assertEvaluation("""currentDateWithTimeZone("HH:mm", "UTC")""", "HH:mm-UTC")
  ...
  fun evaluate_timeManipulation() {
    assertEvaluation("""formatTimestamp(123+456, "format")""", "format 579")
  }
  ...
  private fun assertEvaluation(input: String, expected: Any?) = runTest {
    val context = ScriptContext(FakeBasicScriptContext)
    val actual =
      when (expected) {
        null,
