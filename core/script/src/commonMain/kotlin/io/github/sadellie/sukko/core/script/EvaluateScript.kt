...
private suspend fun evaluateScript(input: String, context: ScriptContext): ASTNode? =
  withContext(Dispatchers.Default) {
    ...
    for (line in inputLines) {
      ...
      // values
      val trees = ASTNode.buildTrees(tokens).map { it.collapse(context) }
      val simplifiedTrees =
        trees.map { tree ->
