...
internal sealed interface ASTNode {
  ...
  companion object {
    internal fun buildTrees(tokens: List<Token3>) = ASTBuilder(tokens).build()

    internal fun buildTreesAndCollapse(tokens: List<Token3>, scriptContext: ScriptContext) =
      buildTrees(tokens).map { it.collapse(scriptContext) }
  }
}
