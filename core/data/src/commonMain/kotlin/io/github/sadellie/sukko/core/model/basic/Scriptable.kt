...
sealed interface Scriptable<V> {
  ...
  sealed interface Global<V> : Scriptable<V> {
    ...
    override suspend fun getValue(
      ...
      if (id in globalIds) {
        Logger.e(tag = TAG) { "getValue: Cyclic link for $this. Globals in link: $globalIds" }
        return defaultValue()
      }
      val global = findGlobal(globals)
      if (global == null) {
        Logger.e(tag = TAG) { "getValue: Failed to find global with id $id in $globals" }
        return defaultValue()
      }
      ...
      val value =
        layerContext.globalValueCache.getOrPut(global, lock = !isInRecursion) {
          Logger.d(tag = TAG) { "getValue: update cache ${global.id} ${global.label}" }
