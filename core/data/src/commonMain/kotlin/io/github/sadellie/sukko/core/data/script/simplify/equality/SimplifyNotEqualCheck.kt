...
import io.github.sadellie.sukko.core.data.script.NotEqualNode
...
internal val SimplifyNotEqualCheck =
  object : SimplificationRule {
    override suspend fun simplify(context: ScriptContext, tree: ASTNode): SimplificationStep? =
      simplifyBottomToTop(SimplificationType.NOT_EQUAL, context, tree) walker@{ currentNode ->
        // only work with not equal node
        if (currentNode !is NotEqualNode) return@walker null
