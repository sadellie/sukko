...
import io.github.sadellie.sukko.core.data.script.LessOrEqualNode
...
internal val SimplifyLessOrEqualCheck =
  object : SimplificationRule {
    override suspend fun simplify(context: ScriptContext, tree: ASTNode): SimplificationStep? =
      simplifyBottomToTop(SimplificationType.LESS_OR_EQUAL, context, tree) walker@{ currentNode ->
        // only work with less or equal node
        if (currentNode !is LessOrEqualNode) return@walker null
