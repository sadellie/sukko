...
import io.github.sadellie.sukko.core.data.script.GreaterNode
...
internal val SimplifyGreaterCheck =
  object : SimplificationRule {
    override suspend fun simplify(context: ScriptContext, tree: ASTNode): SimplificationStep? =
      simplifyBottomToTop(SimplificationType.GREATER, context, tree) walker@{ currentNode ->
        // only work with greater node
        if (currentNode !is GreaterNode) return@walker null
