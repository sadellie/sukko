...
class MediaListenerService : Service(), KoinComponent {
  ...
  override fun onCreate() {
    super.onCreate()
    Logger.d(TAG) { "onCreate service" }
    try {
      val mediaListener by inject<MediaListener>()
      mediaListener.startListening()
    } catch (e: Exception) {
      Logger.e(TAG, e) { "Failed to start media listener" }
    }
  }
  ...
  companion object {
    fun start(context: Context) {
      if (!NotificationListener.canAccessNotifications(context)) {
        Logger.e(TAG) { "Can not start service: notification listener is not allowed" }
        return
      }
      val intent = getServiceIntent(context)
      context.startService(intent)
      Logger.d(TAG) { "Media listener started" }
    }
