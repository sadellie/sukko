...
interface SessionManagerScope {
  ...
  /**
   * If the session still has pending events, and has not been closed (widget deleted) or failed due
   * to error, recreate it to handle those events, without starting a new worker. Returns the new
   * session if it was recreated.
   */
  suspend fun recreateOrClose(session: Session): Session?
}

val UnglanceSessionManager: SessionManager = SessionManagerImpl(SessionWorker::class.java)
...
class SessionManagerImpl(
  ...
  private companion object {
    const val TAG = "UnglanceSessionManager"
  }
  ...
  val scope =
    object : SessionManagerScope {
      ...
      private fun addSessionCloseExisting(session: Session) {
        sessions.put(session.key, session)?.close()
      }
    }
      ...
      object : WorkManagerProxy {
        ...
        override suspend fun workerIsRunningOrEnqueued(
          ...
          WorkManager.getInstance(context)
            ...
            .await()
            .fastAny { it.state in listOf(WorkInfo.State.RUNNING, WorkInfo.State.ENQUEUED) }
      }
  }
